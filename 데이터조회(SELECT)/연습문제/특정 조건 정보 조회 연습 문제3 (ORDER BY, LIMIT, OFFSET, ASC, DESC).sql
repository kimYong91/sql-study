...
SELECT * FROM EMP ORDER BY ENAME ASC LIMIT 5 OFFSET 2;

-- 문제 4: EMP 테이블에서 JOB이 'SALESMAN'인 직원들을 급여(SAL)의 오름차순으로 정렬하여 조회하세요.
SELECT * FROM EMP WHERE JOB IN ('SALESMAN') ORDER BY SAL;

-- 문제 5: EMP 테이블에서 가장 최근에 입사한 직원 3명의 ENAME, JOB, HIREDATE를 조회하세요.
SELECT ENAME, JOB, HIREDATE FROM EMP ORDER BY HIREDATE DESC LIMIT 3;
