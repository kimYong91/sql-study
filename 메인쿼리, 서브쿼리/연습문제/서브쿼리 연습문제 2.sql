...
SELECT actor_id FROM film_actor GROUP BY actor_id HAVING COUNT(film_id) >= 5;
...
SELECT title, (SELECT COUNT(actor_id) FROM film_actor FA WHERE FA.film_id = F.film_id) FROM film F;
...
SELECT MAX(RI) FROM (SELECT COUNT(rental_id) RI FROM rental GROUP BY customer_id) R;