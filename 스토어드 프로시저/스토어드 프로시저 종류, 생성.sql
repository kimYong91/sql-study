...
-- 첫 번째 매개변수(IN)는 저장 프로시저의 INSERT문에 사용되고
-- 두 번쨰 매개변수(OUT)는 해당 변수 (@val)에 SELECT문의 결과가 저장된다.
...
-- 예외처리
-- DECLARE 액션[COUNTINUE || EXIT] HANDLER FOR 오류조건 처리할문장

SELECT * FROM notable;
-- 존재하지 않는 테이블 조회시 에러코드 1146번
-- Error Code: 1146. Table 'store.notable' doesn't exit
