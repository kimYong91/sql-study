...
-- 인덱스 추가 (기본 키 추가로 인덱스 자동 추가)
ALTER TABLE clustertbl
	ADD CONSTRAINT PK_clustertbl_userId
		PRIMARY KEY (userId);

-- 기본 키를 생성하며 클러스터형 인덱스 생성
SHOW INDEX FROM clustertbl;

-- 클러스터형 인덱스(userId)기준으로 오름차순 정렬
SELECT * FROM clustertbl;

-- 클로스터형 인덱스의 루트 페이지(노드) => (리프 페이지 또는 인터널 페이지)의 참조 주소를 가지고 있음
-- 클로스터형 인덱스의 리프 페이지(노드) => 실제 데이터가 저장이 되어있음
...
-- 보조 인덱스가 생성되고, 데이터는 조회 시 userID 컬럼기분으로 정렬되어있지 않음
-- 보조 인덱스의 리프 페이지에서는 인덱스 컬럼(userID) 기준으로 정렬되어 있음
-- 보조 인덱스의 리프 페이지는 실제 데이터를 가지고 있는 것이 아니라,
-- 데이터의 위치를 가리키는 위치 포인터를 가지고 있음
-- 즉, 물리적으로 정렬되어 있지는 않지만 정렬할 수 있게 인덱스가 설정되어 있어서
-- ORDER BY 보조형 설정 컬럼; 이렇게 하면 인덱스가 설정되어 있어 더 빠르게 조회가능
SELECT * FROM secondarytbl;
SHOW INDEX FROM secondarytbl;

-- 클러스터형 인덱스는 범위 검색 시 우수한 성능을 보임
-- 보조 인덱스는 범위 검색을 하기 위해 각기 분산되어 있는 페이지에 접근해야 함

-- 클러스터형 인덱스에서 데이터를 삽입하게 될 경우
-- 삽입 하자마자 인덱스 컬럼 기준으로 페이지를 제정렬(페이지 분할 또한 발생할 수 있음)
-- 페이지 분할은 시스템에 많은 부하를 주게 됨
INSERT INTO clustertbl VALUES ('FNT', '푸니타');
INSERT INTO clustertbl VALUES ('FAI', '카아이');

-- 보조 인덱스에 데이터에 삽입하게 될 경우
-- 실제 데이터 페이지의 빈 부분에 삽입되고 (물지적으로 위치가 변화되는것이 아니라 인덱스의 주소만 입력되고 그냥 입력되어진 정보 다음으로 데이터 삽입)
-- 인덱스 위치만 조정하게 됨 -> 클러스터형 인덱스보다 성능에 주는 부하가 적음
INSERT INTO secondarytbl VALUES ('FNT', '푸니타');
INSERT INTO secondarytbl VALUES ('FAI', '카아이');
...
-- 인덱스 추가
-- 클러스터형 인덱스가 추가되며 실제 데이터 페이지가 정렬
...
-- 혼합된 인덱스의 경우
-- 보조 인덱스에서 컬럼명(이름) 기준으로 검색하여 루트 페이지에서 리프 페이지로 검색
-- 리프 페이지(name 기준으로 정렬되어 있으며 클러스터형 인덱스의 key값을 저장)에서
-- 이름을 기준으로 클러스터형 인덱스의 키값(userID)을 확인 한 후
-- 클러스터형 루트 페이지로 가서 키값으로 검색한 후 원하는 결과를 찾음

-- WHERE 절에 인덱스를 사용한 컬럼의 이름이 있을 경우 해당 인덱스를 사용하게 됨
SELECT addr FROM mixedtbl WHERE name = '임재범';
-- 어떤 인덱스를 사용하였는지 쿼리 확인 (EXPLAIN)
